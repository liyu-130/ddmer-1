...
-- SeedData: 默认管理员账户通过 GitHub OAuth 首次登录自动创建，无需手动插入
-- 确保在 Vercel 环境变量中设置 ADMIN_GITHUB_USERS=你的GitHub用户名
```

> **说明**：SQL 执行后，数据库表结构创建完成。管理员账户通过 GitHub OAuth 首次登录时自动创建，确保安全。

---

## 十一、默认登录信息

数据库初始化完成后，**没有预置的管理员账户**，需要通过 GitHub OAuth 首次登录来创建：

1. 确保 Vercel 环境变量 `ADMIN_GITHUB_USERS` 已设置你的 GitHub 用户名
2. 访问 `https://你的域名.vercel.app/admin/`
3. 点击 GitHub 登录按钮
4. 首次登录后，系统会自动创建管理员账户

| 项目 | 值 |
|------|-----|
| 后台地址 | `https://你的域名.vercel.app/admin/` |
| 登录方式 | GitHub OAuth（推荐） |
| 前提条件 | `ADMIN_GITHUB_USERS` 环境变量包含你的 GitHub 用户名 |

> **安全提示**：
> - 不要将 `ADMIN_GITHUB_USERS` 设置为不信任的用户
> - 首次登录后可在后台修改管理员密码
> - 定期检查后台用户列表

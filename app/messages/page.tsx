...
export default function MessagesPage() {
  ...
  function handleAnonymousLogout() {
    localStorage.removeItem("anonymous_token");
    localStorage.removeItem("anonymous_user");
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 md:py-12">
      ...
      >
        {user ? (
          ...
          <div className="flex items-center gap-2 md:gap-3 px-3 py-2 md:px-5 md:py-3 rounded-2xl bg-white/50 dark:bg-slate-800/60 backdrop-blur-xl border border-white/30 dark:border-white/10">
            <Image
              src={anonymousUser?.avatar || ""}
              alt={anonymousUser?.login || ""}
              width={32}
              height={32}
              className="rounded-full md:w-9 md:h-9"
            />
          ...
          <div className="rounded-2xl bg-white/50 dark:bg-slate-800/60 backdrop-blur-xl border border-white/30 dark:border-white/10 overflow-hidden">
            <div className="p-3 md:p-4 space-y-3">
              <button
                ...
                className="flex items-center gap-2 md:gap-3 w-full hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors rounded-xl px-2 py-1.5"
              >
                <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-slate-900 dark:bg-white flex items-center justify-center flex-shrink-0">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={handleAnonymousLogin} disabled={anonymousLoading}
                  className="px-3 py-2 rounded-xl bg-sky-500 text-white text-xs md:text-sm font-medium hover:bg-sky-600 disabled:opacity-50 transition-colors flex-shrink-0">
                ...
                <div className="flex items-center gap-1.5 md:gap-2 px-3 pt-2 md:px-5 md:pt-3 pb-0 text-[10px] md:text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <span>
                    ...
                    <span className="font-medium text-sky-600 dark:text-sky-400">
                      {replyTo.github_user?.login ?? replyTo.email_user_name ?? "匿名"}
                    </span>
                  </span>
          ...
          <div className="p-3 md:p-4">
            ...
            <div className="flex items-center justify-between mt-2 pt-2 md:mt-3 md:pt-3 border-t border-slate-200/50 dark:border-white/5">
              ...
              <button
                ...
                disabled={(!user && !anonymousUser) || !inputValue.trim() || submitting}
      ...
      <div className="p-3 md:p-5">
        ...
        <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
          {msg.github_user ? (
            ...
            <Image
              src={msg.email_user_avatar || `https://api.dicebear.com/7.x/initials/png?seed=${msg.email_user_name}&backgroundColor=0ea5e9`}
              alt={msg.email_user_name}
              width={32}
              height={32}
              className="rounded-full md:w-9 md:h-9"
            />
          ...
          <div className="flex-1 min-w-0">
            <span className="text-xs md:text-sm font-semibold text-slate-800 dark:text-slate-200">
              {msg.github_user?.login || msg.email_user_name || "匿名用户"}
            </span>
            {msg.github_user?.bio && (
    ...
    <div className="px-3 py-2 md:px-5 md:py-3 border-b border-slate-200/30 dark:border-white/5 last:border-0">
      <div className="flex items-start gap-2 md:gap-3">
        {reply.github_user ? (
          ...
          />
        ) : reply.email_user_name ? (
          <Image
            src={reply.email_user_avatar || `https://api.dicebear.com/7.x/initials/png?seed=${reply.email_user_name}&backgroundColor=0ea5e9`}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 md:gap-2 mb-0.5 md:mb-1">
            <span className="text-[10px] md:text-xs font-semibold text-slate-700 dark:text-slate-300">
              {reply.github_user?.login || reply.email_user_name || "匿名用户"}
            </span>
            <span className="text-[10px] md:text-xs text-slate-400">

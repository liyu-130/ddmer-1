...
function flattenReplies(
  ...
  for (const r of replies) {
    ...
    if (r.replies?.length) {
      map.set(r.id, r.github_user?.login ?? r.email_user_name ?? "匿名用户");
...
export default function MomentsPage() {
  ...
  const [anonymousUser, setAnonymousUser] = useState<{ login: string; avatar: string } | null>(() => {
    if (typeof window === "undefined") return null;
    const saved = localStorage.getItem("anonymous_user");
    if (saved) { try { return JSON.parse(saved); } catch { return null; } }
    return null;
  });
  ...
  useEffect(() => {
    ...
    // 恢复匿名登录状态
    const savedAnonymousToken = localStorage.getItem("anonymous_token");
    if (savedAnonymousToken && !anonymousUser) {
      fetch("/api/auth/anonymous/me", {
        headers: { Authorization: `Bearer ${savedAnonymousToken}` },
      })
        .then((res) => res.json())
        .then((data) => {
          if (data.login) {
            localStorage.setItem("anonymous_user", JSON.stringify(data));
  ...
  function handleGithubLogin() { sessionStorage.setItem("github_redirect", window.location.pathname); window.location.href = "/api/auth/github/login"; }
  function handleLogout() { localStorage.removeItem("github_token"); localStorage.removeItem("github_user"); setUser(null); }
  function handleAnonymousLogout() { localStorage.removeItem("anonymous_token"); localStorage.removeItem("anonymous_user"); setAnonymousUser(null); }
  function cancelReply() { setReplyTo(null); setCommentInput(""); }

  async function handleAnonymousLogin() {
    const name = nicknameInput.trim();
    if (!name) { alert("请输入昵称"); return; }
    if (name.length > 20) { alert("昵称最多 20 个字符"); return; }
    ...
    try {
      ...
      localStorage.setItem("anonymous_user", JSON.stringify(data.user));
      setAnonymousUser(data.user);
      setNicknameInput("");
    } catch (e: any) { alert(e.message); } finally { setAnonymousLoading(false); }
  }
  ...
  async function handleSubmitComment(chatterId: number) {
    if (!commentInput.trim() || submitting || (!user && !anonymousUser)) return;
    setSubmitting(true);
    try {
                        ...
                        {onlyViewId && (<>
                          ...
                          <div className="border-t border-slate-200/50 dark:border-white/5 px-3 md:px-5 py-3 md:py-4" onClick={(e) => e.stopPropagation()}>
                            {user ? (
                              ...
                              <div className="flex items-center gap-2 mb-3 px-2 py-1.5 rounded-xl bg-white/40 dark:bg-slate-700/30">
                                <Image src={anonymousUser.avatar || ""} alt={anonymousUser.login || ""} width={24} height={24} className="rounded-full md:w-7 md:h-7" />
                                <span className="text-xs font-medium text-slate-700 dark:text-slate-300 flex-1">{anonymousUser.login || ""}</span>
                                <button type="button" onClick={handleAnonymousLogout}
                                  className="flex items-center gap-1 text-[10px] text-slate-400 hover:text-red-500 transition-colors">
                                  <LogOut className="w-3 h-3" />退出
                                </button>
                              </div>
                            ) : (
                              <div className="mb-3 rounded-xl bg-white/40 dark:bg-slate-700/30 p-2 space-y-2">
                                <button type="button" onClick={handleGithubLogin}
                                  className="flex items-center gap-2 w-full hover:bg-white/40 dark:hover:bg-slate-700/40 transition-colors rounded-lg px-1 py-1">
                                  <div className="w-5 h-5 rounded-full bg-slate-900 dark:bg-white flex items-center justify-center flex-shrink-0">
                                    <GithubIcon className="w-3 h-3 text-white dark:text-slate-900" />
                                  </div>
                                ...
                                <div className="flex gap-1.5">
                                  ...
                                  <button type="button" onClick={handleAnonymousLogin} disabled={anonymousLoading}
                                    className="px-2 py-1 rounded-lg bg-sky-500 text-white text-[10px] font-medium hover:bg-sky-600 disabled:opacity-50 transition-colors flex-shrink-0">
                                    {anonymousLoading ? "..." : "匿名留言"}
                                  </button>
                                </div>
                              </div>
                            )}
                                ...
                                {replyTo && (
                                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                    <div className="flex items-center gap-1.5 px-2 pt-2 pb-0 text-[10px] text-slate-500 dark:text-slate-400">
                                      ...
                                      <span>回复 <span className="font-medium text-sky-600 dark:text-sky-400">{replyTo.github_user?.login ?? replyTo.email_user_name ?? "匿名"}</span></span>
                              ...
                              <div className="p-2">
                                ...
                                <div className="flex items-center justify-between mt-1 pt-1 border-t border-slate-200/30 dark:border-white/5">
                                  ...
                                  <button type="button" onClick={() => handleSubmitComment(parseInt(moment.id))}
                                    disabled={(!user && !anonymousUser) || !commentInput.trim() || submitting}
      ...
      <div className="p-2 md:p-3">
        <div className="flex items-center gap-2 mb-1.5">
          {comment.github_user ? (
            ...
            <Image src={comment.email_user_avatar} alt={comment.email_user_name} width={22} height={22} className="rounded-full md:w-6 md:h-6" />
          ) : (
          ...
          <span className="text-[10px] md:text-xs font-semibold text-slate-700 dark:text-slate-300">{comment.github_user?.login ?? comment.email_user_name ?? "匿名用户"}</span>
    ...
    <div className="px-2 py-1.5 md:px-3 md:py-2 border-b border-slate-200/20 dark:border-white/5 last:border-0">
      <div className="flex items-start gap-1.5 md:gap-2">
        {reply.github_user ? (
          ...
          <Image src={reply.email_user_avatar} alt={reply.email_user_name} width={18} height={18} className="rounded-full mt-0.5 md:w-5 md:h-5" />
        ) : (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 mb-0.5">
            <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-400">{reply.github_user?.login ?? reply.email_user_name ?? "匿名用户"}</span>

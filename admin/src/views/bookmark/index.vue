...
// 从 Axios 错误中提取后台返回的 message
function getErrorMsg(e: any, fallback = "操作失败") {
...
async function handleSubmit() {
  ...
  try {
    ...
    message(getErrorMsg(e), { type: "error" });
  }
}
...
async function handleDeleteCategory(row: BookmarkCategoryItem) {
  try {
    ...
    message(getErrorMsg(e, "删除失败"), { type: "error" });
  }
}
...
async function handleCatIconUpload(options: any) {
  ...
  try {
    ...
    message(getErrorMsg(e, "上传失败"), { type: "error" });
  } finally {
    catUploading.value = false;
  }
}

async function handleSiteIconUpload(options: any) {
  const file = options.file;
  if (!file) return;
...
const siteColumns: TableColumnList = [
  ...
  { label: "链接", prop: "url", minWidth: 180, slot: "url" },
  {
    label: "描述",
    prop: "description",
    minWidth: 150,
...
function openSiteDialog(title: string, row?: BookmarkSiteItem) {
  ...
  // 加载分类选项供下拉选择
  categoryOptions.value = dataList.value.map((c) => ({ id: c.id, name: c.name }));
  if (row) {
    siteForm.value = {
      id: row.id,
          ...
          <div class="flex items-center gap-4 w-full">
            ...
            <div class="flex flex-col gap-2 flex-1">
              <el-upload
                ...
                :http-request="handleCatIconUpload"
                accept="image/*"
              >
      ...
      <div v-loading="sitesLoading">
        ...
        >
          ...
          <template #url="{ row }">
            ...
            >
              {{ row.url.length > 30 ? row.url.slice(0, 30) + "..." : row.url }}
            </a>
        ...
        <el-form-item label="图标">
          <div class="flex items-center gap-4 w-full">
            ...
            <div class="flex flex-col gap-2 flex-1">
              <el-upload
                :show-file-list="false"
                :http-request="handleSiteIconUpload"
                accept="image/*"
              >

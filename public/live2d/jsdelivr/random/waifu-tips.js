...
!function () {
    ...
    hitokoto: {
      ...
      callback: function () {
        ...
        fetch("https://v1.hitokoto.cn?" + Date.now())
          ...
          .catch(() => { showMessage("一言获取失败了，请检查网络哦～", 3000, 9); });
      }
    },
    asteroids: {
      icon: SVG.asteroids,
    ...
    photo: {
      ...
      callback: () => {
        ...
        if (canvas && canvas.tagName === "CANVAS") {
          ...
          showMessage("截图失败了呢...", 3000, 9);
        }
      }
    },
    info: { icon: SVG.info, callback: () => { open("/about", "_self"); } },
    quit: {
      icon: SVG.quit,
      callback: () => {
  ...
  function init(config) {
    ...
    for (const name of toolList) {
      ...
      const title = toolTitles[name] || name;
      document.getElementById("waifu-tool").insertAdjacentHTML("beforeend",
        `<span id="waifu-tool-${name}" title="${title}">${icon}</span>`
      );
